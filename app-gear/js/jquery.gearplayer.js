  ...
  //if (window.location.protocol == 'file:') { alert('Whoops! You need a Webserver to run Gear Player. Get a local one here: https://www.apachefriends.org/'); }
    ...
    $.fn.gearPlayer.options = {
	    ...
	    "soundcloudOverwrite":false,  // this overwrites the metadata of soundcloud, handy if you're out of control
	    "autoPlay": true,  // launches the player on start and plays the first song, doesn't work on mobile
	    "shuffle": false, // this basically mixes the order of the songs
	    ...
	    "equalizerSize":160, // how big the equalizer should be rendered
	    ...
	    "outerRay": 55, // this value refers to the outside ray of the player
	    "innerRay": 45, // and this the part inside of it
	    "outerPadding": 10, // how much space to leave outside of the ring
	    "innerPadding": 10, // and inside of it
	    "sectorPadding": 8, // the spacing between the sectors
	    ...
	    "loadedThickness": 0, // how thick the loaded ring should be
	    "progressThickness": 0, // same as above but for the progress ring
	    "timeColor":"#FFFFFF", // this colors the time text
	    "timeSize": 0,  // the size of the time text
	    "randomColors": false,  // this overwrites all sector colors and randomizes them
	    "textColor": "#FFFFFF", // that's the color of the song names around the player
	    "overColor": "#F2F2F2",  // the color when hovering
      "titleSize": 12, // the title size around the sectors
	    "authorSize": 10,  // the author size right below the title
	    "dockToRight":true // this basically docks the miniplayer to the right
    };
          ...
          _gPlayer.sound = soundManager.createSound({
               ...
               whileplaying: function() {
					...
					if (!_gPlayer.sound.paused && $('.gearWrap').css('visibility')!='hidden') 
					{
						...
						//console.log(_gPlayer.position/_gPlayer.duration);
						...
						{ 
                        	if (_gEqualizer.useSoundCloudWave && gSet.equalizer) 
							{ 
						...
						{ 
                        	if (_gEqualizer.useSoundCloudWave && gSet.peak) 
							{ 
					...
					{
                    	_gPlayer.position = this.position;
						_gPlayer.duration = this.duration;
                    	_gMiniPlayer.update(this.position,_gPlayer.loaded ? this.duration : this.durationEstimate);
						
						var percentPlayer=(_gPlayer.position/_gPlayer.duration)*100;
						// document.getElementById('rangeslider').value = percentPlayer; 
                  	}
  ...
  create: function(gSet){
    ...
    _gMiniPlayer.ray = ray;
        ...
        c.customAttributes.arc = function (value, total, R, type) {
			if(total == 0)
			{
				var total = value; // modification manuelle pour retirer erreur
			}
...
var _gConsole = {
    create: function(gSet) {
		...
		timeDiv.style.display = playDiv.style.display = btnDiv.style.display = "none";
    ...
    showPlaylist: function(){
      if (!$('#_gPlayer').hasClass('docked') /*&& !_gPlayer.scrubbing*/ && $(window).width() < 767) { 
				...
				TweenMax.to($('.gear'), 0.5, {left: '0%', transformOrigin:"center center", ease:Cubic.easeInOut});
    ...
    hidePlaylist: function(){
      if ($('#_gPlayer').hasClass('docked') /*&& !_gPlayer.scrubbing*/ && $(window).width() < 767) { 
				...
				TweenMax.to($('.gear'), 0.5, {left: '50%', ease:Cubic.easeInOut});
				TweenMax.to($('#gearPlay'), 0.5, {left: '0%', ease:Cubic.easeInOut});
...
var _gInterface = {
  create: function(gSet){
        ...
        currentDiv.id = '_gCurrent'; ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// CURRENT /////////////////////////
  ...
  show: function(){
    ...
    if (!_gInterface.open || $('.stage').css('display')=='none') {  //double flag whenever one fails
	  ...
	  $("#menu li:first").addClass("mm-selected");
               ...
               function handleClickEvent() {
                ...
                function handleTouchEvents() {
                    ...
                    piece.touchstart(function(e) { 
                        e.preventDefault(); 
						/*if (tracker.getVelocity("rotation")==0) {*/ // Retirer
                        _gConsole.clicked = true;
                        doSector(j);
                        /*}*/ // Retirer
                    });
...
function updateOutput(figure, activate) {
	...
	if (rfigure!=null && rfigure>0) 
	{
		_gPlayer.setPosition(rfigure);
	}
