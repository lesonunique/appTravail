Dedicace = {
    ...
    launch : function() { // ETAPE 1 : LANCEMENT MYSUN PAGE AUJOURD'HUI/DEMAIN + INITIALISATION URLS --> choix jour puis direction selectDay()
    ...
    selectDay : function(p_param) { // ETAPE 2 : RECUPERER DATE HEURE ACTUELLE / RECUPERER DATE DEMAIN --> displayHourAvailable() // INIT URL
		...
		else if(Dedicace._choixMysun == 2)
		{
			Dedicace._url_get_hour = Flux._frap_flux.racine_services+Flux._frap_flux.repertoire_dedicace+'/selection_auditeur_100mysun/selecteur_heures.php';
			Dedicace._url_get_min = Flux._frap_flux.racine_services+Flux._frap_flux.repertoire_dedicace+'/selection_auditeur_100mysun/selecteur_minutes.php';
			Dedicace._url_get_list = Flux._frap_flux.racine_services+Flux._frap_flux.repertoire_dedicace+'/selection_auditeur_100mysun/recherche_creation_tableau.php';
			Dedicace._url_get_info = Flux._frap_flux.racine_services+Flux._frap_flux.repertoire_dedicace+'/selection_auditeur_100mysun/traitement_musiqueEnAvant.php';
			Dedicace._url_send_dedicace = Flux._frap_flux.racine_services+Flux._frap_flux.repertoire_dedicace+'/selection_auditeur_100mysun/validation.php';
		}
		else
		{
			Dedicace._url_get_hour = Flux._frap_flux.racine_services+Flux._frap_flux.repertoire_dedicace+'/selection_auditeur/selecteur_heures.php';
    ...
    valideDedicace : function() {			
		if(nbPlaylistActuel>0 && Dedicace._search_words=='')
		{
			...
			$.ajax
			({
				url: 'http://lesonunique.com/user/'+ConnectLogin._idUsr+'/myplaylist2/nodes.json', 		    
				type: 'GET',
				dataType: 'json',
				headers: 
				{
				...
				success: function(data) 
				{
					...
					({
						...
						success: function(jsonActuelMerge) 
						{
							...
							/*jembe.http.post({
								url: 'http://www.lesonunique.com/dedicace/selection_auditeur_100mysun/test_json.php',
								data: 	'artiste='
										...
										+'&mode='
										+'&musique='+Dedicace._tableauPlaylist,
    ...
    displayTitles : function() {
        ...
        for(var i=0;i<Dedicace._json_list.length;i++) {
            ...
            content +=                          '<div class="play" onclick="newSonTemp('+i+',\'ded\')"><img src="images/player/play.png" /></div>'+
                                                '<div class="pause" onclick=""><img src="images/player/pause.png" /></div>';
            content +=                      '</div>' +
                                            '<div class="line_play_dedicace" id="line_'+i+'_play_dedicace"><div class="timer_dedicace"></div></div>' +
                                        '</td>' +
                                        '<td valign="top" style="padding-top:15px;">' +
                                            '<strong style="text-transform:uppercase;">'+Dedicace._json_list[i].artiste+' - '+Dedicace._json_list[i].titre+'</strong><br /><small class="diffusionOk">'+Dedicace._json_list[i].fld_next_available+'</small>' +
                                        '</td>' +
                                    '</tr>' +
                                    '<tr>' +
                                        '<td colspan="2">' +
    ...
    callback_dedicacer : function(msg) {
        ...
        content +=  '<tr>' +
					...
					'<tr>' +
                        '<td>' +
							'<input type="text" placeholder="Votre prénom" class="prenom" value="" id="mon_identifiant_pour_dedicacer">'+ 
            ...
            var content =   '<div id="line_addressbook_'+Dedicace._num_line_adressbook+'" style="padding:5px;background:#fff;margin-bottom:2px;">' +
                                '<table cellpadding="0" cellspacing="0" border="0" width="100%">' +
                                    '<tr>' +
                                        ...
                                        '</td>' +
                                        '<td align="center" width="30" onclick="Dedicace.delete_adressbook('+Dedicace._num_line_adressbook+');"><a id="croixNewNum" href="javascript:"><img src="images/croixJaune.png" /></a></td>' +
                                    '</tr>' +
                                '</table>' +
                            '</div>';
    ...
    callback_addressbook : function() {
        ...
        var content =   '<div id="line_addressbook_'+Dedicace._num_line_adressbook+'" style="padding:5px;background:#fff;margin-bottom:2px;">' +
                            '<table cellpadding="0" cellspacing="0" border="0" width="100%">' +
                                '<tr>' +
                                    '<td>' +
										'<div>'+Dedicace._adressBook_name+'</div>' +
                                        '<div><input type="text" value="'+Dedicace._adressBook_phone+'" id="phone_number_'+Dedicace._num_of_tel+'" readonly="readonly"  /></div>' +
                                    '</td>' +
                                    '<td align="center" width="30" onclick="Dedicace.delete_adressbook('+Dedicace._num_line_adressbook+');"><a href="javascript:"><img src="images/croixJaune.png" /></a></td>' +
                                '</tr>' +
                            '</table>' +
                        '</div>';
